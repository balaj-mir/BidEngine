...
def generate_exact_capabilities():
    data_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data")
    os.makedirs(data_dir, exist_ok=True)

    raw_data = [
        ("CAP-001", "Cybersecurity", "Project 1: Cybersecurity deployment for client", "ISO 27001", 2023, "PKR 15M", 34, "International"),
        ...
        ("CAP-046", "LMS Development", "Project 46: LMS Development deployment for client", "ISO 27001", 2023, "PKR 102M", 25, "International"),
    ...
    capabilities = []
    for cap_id, domain, summary, cert, year, value_str, duration, client_type in raw_data:
        # Parse value_str e.g., "PKR 15M" -> 15000000.0, "PKR 8M" -> 8000000.0
        val = 0.0
        if "M" in value_str:
            num = float(value_str.replace("PKR", "").replace("M", "").strip())
            val = num * 1000000.0
        ...
        capabilities.append({
            "id": cap_id,
            "title": summary,
            "description": f"Completed deployment of {domain} system. Details: {summary}. Client Type: {client_type}. Duration: {duration} months.",
            "sector": domain,
            ...
            "team_size": 12,
            "keywords": [domain.lower(), client_type.lower()] + ([c.lower() for c in certifications]),
            "outcome": "Project successfully verified and approved by the evaluation committee.",
    ...
    print("Overwrote capability_library.json with the exact dataset from the user image!")

if __name__ == "__main__":
    generate_exact_capabilities()
